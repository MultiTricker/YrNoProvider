...
- hodinová předpověď pro aktuálních 12 hodin (konfigurovatelně až 48 h)
- předpověď po sekcích (dopoledne, odpoledne, večer, noc) pro dnešek a zítřek (konfigurovatelně až 9 dní)
...
Chování (množství vrácených dat) lze ovlivnit parametry **mode**, **days** a **hours**:
...
**days** – počet dní v sekcích (výchozí: `1`):
- `days=1` – dnes + zítřek (výchozí chování)
- `days=2` – dnes + zítřek + pozítří
- `days=N` – až `days=9` (limit API yr.no)
- Každý den navíc se vrátí jako jedna sekce pokrývající 06:00–20:00, pojmenovaná dle dne v týdnu: `po_den`, `ut_den`, `st_den`, `ct_den`, `pa_den`, `so_den`, `ne_den`.

**hours** – počet hodinových záznamů (výchozí: `12`):
- Povolený rozsah `hours=1` až `hours=48`.
- Yr.no poskytuje hodinová data přibližně na 48 hodin dopředu, pak přechází na 6hodinové intervaly.

Příklady volání:
```
/yrno/forecast?lat=50.853&lon=15.321&alt=500
/yrno/forecast?lat=50.853&lon=15.321&alt=500&days=5
/yrno/forecast?lat=50.853&lon=15.321&alt=500&days=3&hours=24
/yrno/forecast?lat=50.853&lon=15.321&alt=500&mode=2&hours=48
/yrno/forecast?lat=50.853&lon=15.321&alt=500&mode=1&days=7
```
...
- **po_den / ut_den / st_den / ct_den / pa_den / so_den / ne_den** (06:00-20:00) – pro dny 2 a dál dle parametru `days`

Sekce se vrací podle denní doby. Sekce zapsané kurzívou se vrací pouze tehdy, pokud už je večer (později než 18:00); přes den se vrací sumární **zitra_den**. Sekce **zitra_noc** se vrací jen pokud probíhá "dnešní noc", tedy je mezi půlnocí a ránem. Sekce pro další dny (po, ut …) se přidají na konec pole vždy, pokud je `days ≥ 2`.
